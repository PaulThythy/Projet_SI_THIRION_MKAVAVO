...
/**Zoom Camera**/
double base_ortho_x=2.0, base_ortho_y=2.0;
...
int main(int argc,char **argv)
{
  ...
  glLoadIdentity();
  glOrtho(-base_ortho_x, base_ortho_x, -base_ortho_y, base_ortho_y, -20.0, 20.0);
  ...
  glEnable(GL_LIGHT0);
    ...
    //GL_LIGHT1 se joue pendant l'animation
  //glEnable(GL_LIGHT2);
...
void clavier(unsigned char touche,int x,int y)
{
  ...
  case '1': /**Touche animation*/
    if(animVal==1) {
        animVal++;
        glEnable(GL_LIGHT1);
        /*TODO Changer la texture du visage lors de l'animation si trouve une bonne texture
            Ne pas oublier de la remettre en place a la fin
        */
    }
  break;
...
void souris(int bouton, int etat,int x,int y)
{
  ...
  if(bouton==MOUSE_SCROLL_UP)
        zoomIn();
  if(bouton==MOUSE_SCROLL_DOWN)
        zoomOut();
}
...
void zoomIn(){
    ...
    if(base_ortho_x>0.3 && base_ortho_y>0.3){
        base_ortho_x-=zoom_factor;
        base_ortho_y-=zoom_factor;
